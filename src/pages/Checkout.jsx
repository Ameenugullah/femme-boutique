...
// Public key placeholder — replace with your real Stripe/PayStack key
...
export default function Checkout() {
  ...
  const handlePayment = async (e) => {
    ...
    // In production: integrate with Stripe.js using PAYMENT_PUBLIC_KEY
    console.log('Processing payment with key:', PAYMENT_PUBLIC_KEY);
      ...
      <div className="min-h-screen bg-cream-50 pt-24 flex items-center justify-center px-6">
        <div className="text-center max-w-lg animate-fade-up">
          ...
          <p className="font-body text-charcoal-700/70 mb-3 leading-relaxed">
            Thank you for your order, <strong>{form.firstName}</strong>! We've sent a confirmation to <strong>{form.email}</strong>.
          </p>
          <p className="font-body text-sm text-charcoal-700/50 mb-3">
            Your order will be shipped to {form.address}, {form.city}.
          </p>
          <div className="bg-white border border-sand-200 p-4 mb-8 text-left">
            <p className="font-body text-sm font-medium text-charcoal-800 mb-1">Order #FB{Math.floor(Math.random() * 90000) + 10000}</p>
            <p className="font-body text-xs text-charcoal-700/50">Estimated delivery: 5–7 business days</p>
              ...
              <div className="bg-white border border-sand-200 p-8 shadow-soft animate-fade-in">
                ...
                <div className="grid sm:grid-cols-2 gap-5 mb-5">
                  <FormField label="First Name" id="firstName" error={errors.firstName} required>
                    <input id="firstName" value={form.firstName} onChange={set('firstName')} className="input-field" placeholder="Sophie" />
                  </FormField>
                  <FormField label="Last Name" id="lastName" error={errors.lastName} required>
                    <input id="lastName" value={form.lastName} onChange={set('lastName')} className="input-field" placeholder="Martin" />
                  </FormField>
                </div>

                <div className="grid sm:grid-cols-2 gap-5 mb-5">
                  <FormField label="Email" id="email" error={errors.email} required>
                    <input id="email" type="email" value={form.email} onChange={set('email')} className="input-field" placeholder="sophie@example.com" />
                  </FormField>
                  <FormField label="Phone" id="phone" error={errors.phone}>
                    <input id="phone" type="tel" value={form.phone} onChange={set('phone')} className="input-field" placeholder="+1 (555) 000-0000" />
                  </FormField>
                </div>

                <div className="border-t border-sand-100 pt-6 mt-6 mb-5">
                  <h3 className="font-body font-medium text-charcoal-800 mb-5">Shipping Address</h3>
                  <FormField label="Street Address" id="address" error={errors.address} required>
                    <input id="address" value={form.address} onChange={set('address')} className="input-field mb-3" placeholder="123 Rue de la Paix" />
                    <input value={form.address2} onChange={set('address2')} className="input-field" placeholder="Apartment, suite, etc. (optional)" />
                  </FormField>
                </div>
                ...
                <div className="grid sm:grid-cols-3 gap-5 mb-5">
                  <div className="sm:col-span-2">
                    <FormField label="City" id="city" error={errors.city} required>
                      <input id="city" value={form.city} onChange={set('city')} className="input-field" placeholder="New York" />
                    </FormField>
                  </div>
                  <FormField label="State" id="state" error={errors.state}>
                    <input id="state" value={form.state} onChange={set('state')} className="input-field" placeholder="NY" />
                  </FormField>
                </div>

                <div className="grid sm:grid-cols-2 gap-5 mb-6">
                  <FormField label="ZIP / Postal Code" id="zip" error={errors.zip} required>
                    <input id="zip" value={form.zip} onChange={set('zip')} className="input-field" placeholder="10001" />
                  </FormField>
                  <FormField label="Country" id="country" error={errors.country}>
                    <div className="relative">
                      <select id="country" value={form.country} onChange={set('country')} className="input-field appearance-none">
              ...
              <form onSubmit={handlePayment} className="bg-white border border-sand-200 p-8 shadow-soft animate-fade-in">
                ...
                <div className="flex items-center gap-2 mb-6 p-4 bg-sand-50 border border-sand-200">
                  {['VISA', 'MC', 'AMEX', 'PayPal'].map(card => (
                    <span key={card} className="font-body text-xs font-semibold text-charcoal-700/60 border border-sand-200 bg-white px-2 py-1">
                      {card}
                    </span>
                  ))}
                ...
                <div className="space-y-5">
                  <FormField label="Cardholder Name" id="cardName" error={errors.cardName} required>
                    <input id="cardName" value={form.cardName} onChange={set('cardName')} className="input-field" placeholder="Sophie Martin" />
                  </FormField>
                ...
                <div className="mt-8 p-4 bg-blush-50 border border-blush-100 mb-6">
                  <p className="font-body text-xs text-blush-600">
                    🔒 Your payment is processed securely. We use <strong>{PAYMENT_PUBLIC_KEY.slice(0, 12)}...</strong> for encrypted transactions.
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className={`w-full py-4 font-body font-medium text-base flex items-center justify-center gap-3 transition-all duration-300 ${
                    loading
                      ? 'bg-charcoal-700/50 text-white cursor-not-allowed'
                      : 'bg-blush-500 hover:bg-blush-600 text-white hover:scale-[1.01]'
                  }`}
                >
                  {loading ? (
                    <>
                    ...
                    <>
                      <Lock size={18} />
                      Pay ₦${total.toLocaleString("en-NG")} Securely
                    </>
                  )}
                </button>
          ...
          <div className="lg:sticky lg:top-28">
            <div className="bg-white border border-sand-200 p-6 shadow-soft">
                ...
                {cartItems.map(item => (
                  <div key={item.key} className="flex gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="font-body text-xs font-semibold text-charcoal-800 mt-1">${(item.price * item.quantity).toFixed(2)}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="border-t border-sand-200 pt-4 space-y-2 mb-4">
                ...
                <div className="flex justify-between font-body text-sm">
                  <span className="text-charcoal-700/60">Tax</span>
              ...
              <div className="border-t border-sand-200 pt-4">
                <div className="flex justify-between font-body font-semibold text-base">
                  <span>Total</span>
                  <span className="text-blush-500">${total.toFixed(2)}</span>
                </div>
              </div>
            </div>
          </div>
