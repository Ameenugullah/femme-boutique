    ...
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-cream-50/95 backdrop-blur-md shadow-soft py-3' : 'bg-transparent py-5'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1 sm:gap-2">
            ...
            {searchOpen ? (
              <form onSubmit={handleSearch} className="flex items-center animate-fade-in">
                <input autoFocus type="text" value={searchQuery}
                  ...
                  className="border-b border-charcoal-700 bg-transparent font-body text-sm py-1 px-2 w-36 sm:w-44 focus:outline-none focus:border-blush-500 transition-colors"
                />
              ...
              <button onClick={() => setSearchOpen(true)}
                className="p-2 text-charcoal-700 hover:text-blush-500 transition-colors duration-200" aria-label="Search">
                <Search size={20} />
              </button>
            )}
          </div>
          ...
          {/* Admin Icon */}
          <Link to="/admin" className="p-2 text-charcoal-700/40 hover:text-blush-500 transition-colors duration-200" aria-label="Admin" title="Admin Dashboard">
            <Settings size={18} />
          </Link>

          {/* Mobile menu */}
          <button className="md:hidden p-2 text-charcoal-700 hover:text-blush-500 transition-colors"
